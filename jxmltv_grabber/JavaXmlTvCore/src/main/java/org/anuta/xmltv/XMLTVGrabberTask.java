...
public class XMLTVGrabberTask implements Runnable {
    ...
    private final static Log log = LogFactory.getLog(XMLTVGrabberTask.class);

    private int day = 0;

    private long maxOverlap = 10; // 10

    // minutes

    private int overlapFixMode = 0; // 0 -
    ...
    public void run() {
        ...
        for (Program shortProgram : programs) {
            ...
            if (prog == null) {
                ...
                if (fullProgram.getGanre() != null) {
                    ...
                    if (getImdbAccess() != null) {
                        ...
                        if (getStarRatingGanres().contains(ganre.toLowerCase())) {
                            ...
                            if (log.isDebugEnabled()) {
                                log.debug("Checking imdb for rating of " + fullProgram.getTitle() + " "
                                    + fullProgram.getSubTitle() + " " + year);
                            }
                            MovieRating starRating =
                                getImdbAccess().getRating(fullProgram.getTitle(), fullProgram.getSubTitle(), year);
                            if (starRating != null) {
                                if (log.isDebugEnabled()) {
                ...
                // credits
                if ((fullProgram.getActors() != null) || (fullProgram.getDirectors() != null)
                    || (fullProgram.getPresentors() != null)) {
                    if (log.isDebugEnabled()) {
